...
import stidx.tables.ForeignInvestTrans;
import stidx.tables.StockCode;

public class BulkInsertForeignInvest extends HibernateDaoSupport implements
        BulkInsertModule {
    Logger log = LogManager.getLogger(BulkInsertForeignInvest.class);
    ...
    public void insertBulkRaw(String date, String time, String data) throws Exception {
        ...
        if (stockCodeDao.select(item[11]) == null) {
            ...
            if (item.length > 12)
                stockCode.setCategoryName(item[12]);
            stockCodeDao.insert(stockCode);
        }

        ForeignInvestTrans tr = new ForeignInvestTrans();
        tr.setGsDate(date);
        tr.setGsTime(time);
    ...
    public void insertOrUpdate(ForeignInvestTrans foreignInvestTrans) {
        getHibernateTemplate().saveOrUpdate(foreignInvestTrans);
    }

    public void delete(ForeignInvestTrans foreignInvestTrans) {
        getHibernateTemplate().delete(foreignInvestTrans);
    }
